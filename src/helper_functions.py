...
def preprocess_main_key(img):
    # if img.height < 50:
    #     img = img.resize((img.width * 2, img.height * 2))
    # img.save("C:/Users/ynjason/Desktop/otherProjects/HSR-Relic/testImages/testBefore.png")
    for x in range(img.width):
        for y in range(img.height):
            pixel = img.getpixel((x, y))
            if (
                200 < pixel[0] < 245 and 120 < pixel[1] < 170 and 50 < pixel[2] < 90
            ):
                # img.putpixel((x, y), (255, 255, 255))
                pass
            else:
