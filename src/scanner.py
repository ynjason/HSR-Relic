...
from character_relic_scanner import CharacterRelicScanner
...
class HSRScanner:
    ...
    async def start_scan(self):
        if not any(
            [
                ...
                self._config["scan_character_relics"],
                self._config["cursor_testing"]
            ]
        ):
        ...
        characters = []
        character_relics = []
        if self._config["cursor_testing"]:
            self.cursor_test()
        else:
            ...
            if self._config["scan_character_relics"] and not self.interrupt.is_set():
                character_relics = self.scan_character_relics()

        if self.interrupt.is_set():
            await asyncio.gather(*light_cones, *relics, *characters, *character_relics)
            return
        ...
        return {
            ...
            "character_relics": await asyncio.gather(*character_relics)
        }
    ...
    def cursor_test(self):
        while not self.interrupt.is_set():
            self._nav.print_mouse_position()
            time.sleep(1)
        print(self._screenshot.screenshot_character_relic_stats())
    ...
    def scan_character_relics(self):
        char_relic_scanner = CharacterRelicScanner(
            self._screenshot, self.logger, self.interrupt, self.update_progress
        )
        nav_data = char_relic_scanner.NAV_DATA[self._aspect_ratio]
        ...
        # Get character count
        total_character_count = self._screenshot.screenshot_character_count()
        total_character_count = image_to_string(total_character_count, "0123456789/", 7)
        try:
            total_character_count, _ = total_character_count.split("/")
            total_character_count = int(total_character_count)
        except ValueError:
            raise ValueError(
                "Failed to parse character count."
        ...
        while character_count > 0:
            ...
            try:
                path, character_name = map(str.strip, character_name.split("/"))
                character_name, path = char_relic_scanner.get_closest_name_and_path(
                    character_name, path
                )
                stats_dict["name"] = character_name

                # Get relics
                self._nav.move_cursor_to(*nav_data["relics_button"])
                time.sleep(0.1)
                self._nav.click()
                time.sleep(1)

                stats_dict["relics"] = []
                # Get stats for each relic for the character
                for relicCoord in nav_data["relics"]:
                    self._nav.move_cursor_to(*relicCoord)
                    time.sleep(0.5)
                    self._nav.click()
                    time.sleep(1)
                    stats_dict["relics"].append(self._screenshot.screenshot_character_relic_stats())
                    self._nav.key_press(Key.esc)
                    time.sleep(1)

                task = asyncio.to_thread(char_relic_scanner.parse, stats_dict)
                tasks.add(task)
            except Exception as e:
                self.logger.emit(
                    f'Failed to parse character {character_name}. Got "{e}" error. Skipping...'
            ...
            elif total_character_count <= nav_data["chars_per_scan"]:
                x, y = nav_data["char_end"]
                x -= nav_data["offset_x"] * (total_character_count - 2)
            else:
                x, y = nav_data["char_start"]
                i += 1
                x = x + nav_data["offset_x"] * i
